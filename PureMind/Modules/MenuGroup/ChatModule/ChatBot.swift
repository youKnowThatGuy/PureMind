...
class ChatBot{
    ...
    let helloMessage = "Привет, я Майнди - твой карманный психолог. Со мной ты можешь поделиться тем, что тебя беспокоит, описав ситуацию и свои эмоции по отношению к ней. Я постараюсь помочь разобраться в произошедшем и подсказать, как легче пережить это. \n \nИмей ввиду, я - лишь помощник. Если ты понимаешь, что тебе было бы комфортнее пообщаться со специалистом, ты всегда можешь к нему обратиться, нажав кнопку «хочу к специалисту» в меню снизу. Помни, просить о помощи - это нормально и ты молодец, что решился на это! Приступим?"
    var path = [Int]()
    ...
    private func routeOneChoiceOneTwo(cases: [Int]) -> [String]{
        ...
        case let x where x >= cases[1] + 2:
            ...
            case 1:
                ...
                return ["1", String("?ьтатобароп яндогес летох ыт ыб меч даН !ошороХ".reversed()), String("ииняотсос меовс ан ясьтичотодерсос учох икат есВ".reversed()), String("елет в имяинещущо дан ьтатобароп учоХ".reversed()), String("яинещущо иицомэ иовс ьтасипо отсорп учоХ".reversed())]
            case 2:
                return ["К сожалению, данная функция пока недоступна."]
            default:
                return ["error"]
            }
    ...
    private func bigRouteTwo() -> [String]{
        ...
        case 3:
            ...
            case 1:
                return ["1", "К сожалению, данная функция пока недоступна."]
            default:
                return ["error"]
            }
    ...
    private func routeTwoChoiceZeroZero() -> [String]{
        switch path.count {
        case 4:
            return ["textRequiered", String("?ьсоличулс йобот с отч ,ьтазакссар ьшежом ,ьтяноп ябет ешчул ыботч оН .имяицомэ и имяинещущо имиовт с алачанс меатобароп йавад ,катИ".reversed()), "Возможность написать, что случилось: -введите текст-"]
